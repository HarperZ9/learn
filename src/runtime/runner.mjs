...
export async function run(workflow, { driver, allowIrreversible = false, fromSeq = 0, ledger = new Ledger(), humanAttest = null } = {}) {
  ...
  for (let i = fromSeq; i < workflow.steps.length; i++) {
    ...
    const g = decide(step, { sealedKinds, allowIrreversible });
    if (g.decision === "deny") {
    ...
    try {
      ...
      ledger.append({ kind: "step", seq: i, stepKind: step.kind, digest: obs.digest, summary: res.payload });
...
export async function resume(workflow, { driver, ledger, haltedAt, allowIrreversible = false, humanAttest = null } = {}) {
  return run(workflow, { driver, ledger, allowIrreversible, fromSeq: haltedAt + 1, humanAttest });
}
